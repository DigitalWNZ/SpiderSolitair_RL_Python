...
class SpiderSolitaireFeaturesExtractor(BaseFeaturesExtractor):
    """
    Custom feature extractor for Spider Solitaire that processes the tableau
...
def train_spider_solitaire(total_timesteps=2_000_000, n_envs=4, learning_rate=3e-4):
    ...
    log_dir = f"./logs/spider_solitaire_{timestamp}"
    model_dir = f"./models/spider_solitaire_{timestamp}"
    os.makedirs(log_dir, exist_ok=True)
    os.makedirs(model_dir, exist_ok=True)
    
    print("Training Full PPO")
    print("Network architecture: 3 Conv layers (32, 64, 64 channels), 2 FC layers (256, 256 neurons)")
    print("Policy/Value networks: 256→256 (two layers each)")
    ...
    policy_kwargs = dict(
        features_extractor_class=SpiderSolitaireFeaturesExtractor,
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq=50000,
        save_path=model_dir,
        name_prefix='spider_solitaire_ppo',
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Train or evaluate Spider Solitaire PPO agent')
