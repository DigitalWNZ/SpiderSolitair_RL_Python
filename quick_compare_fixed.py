...
def quick_train_dqn(episodes=100, max_steps_per_episode=500):
    """Quick DQN training with fixed environment."""
    print("\n" + "="*50)
    ...
    for episode in range(episodes):
        ...
        while True:
            ...
            if done:
                ...
                if terminated:
                    print(f"[DQN] Episode {episode + 1} - WON! Reward: {episode_reward:.2f}")
                elif truncated:
                    print(f"[DQN] Episode {episode + 1} - Truncated at step {episode_length}. Reward: {episode_reward:.2f}")
        ...
        if (episode + 1) % 20 == 0:
            ...
            print(f"[DQN] Progress: Episode {episode+1}/{episodes}, Avg Reward: {avg_reward:.2f}, "
...
def quick_train_a2c(episodes=100, max_steps_per_episode=500):
    """Quick A2C training with fixed environment."""
    print("\n" + "="*50)
    ...
    while episodes_completed < episodes:
        ...
        if episodes_completed % 20 == 0 and episodes_completed > 0:
            ...
            print(f"[A2C] Progress: Episodes {episodes_completed}/{episodes}, "
...
def plot_quick_comparison(results_list):
    ...
    os.makedirs("results/quick_comparison_fixed", exist_ok=True)
    
    fig, axes = plt.subplots(2, 2, figsize=(12, 10))
    fig.suptitle('Spider Solitaire RL Algorithm Quick Comparison (Fixed)', fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig('results/quick_comparison_fixed/comparison.png', dpi=150, bbox_inches='tight')
    plt.close()
    
    print("Plots saved to results/quick_comparison_fixed/comparison.png")
...
def generate_quick_report(results_list):
    ...
    report = []
    report.append("# Spider Solitaire RL Quick Comparison Report (Fixed)")
    ...
    report.append("### Environment Fixes:")
    ...
    # Save results
    with open('results/quick_comparison_fixed/report.md', 'w') as f:
        f.write('\n'.join(report))
    
    # Save raw data
    with open('results/quick_comparison_fixed/raw_results.json', 'w') as f:
        json.dump(results_list, f, indent=2)
    
    print("\nReport saved to results/quick_comparison_fixed/report.md")
    print('\n'.join(report))


def main(episodes=100, max_steps_per_episode=500):
    """Run quick comparison of all algorithms."""
    print("Spider Solitaire RL Quick Comparison (Fixed)")
    print(f"Training each algorithm for {episodes} episodes...")
    print(f"Max steps per episode: {max_steps_per_episode}")
    print("="*50)
    ...
    # Train DQN
    try:
        dqn_results = quick_train_dqn(episodes=episodes, max_steps_per_episode=max_steps_per_episode)
    ...
    # Train A2C
    try:
        a2c_results = quick_train_a2c(episodes=episodes, max_steps_per_episode=max_steps_per_episode)
    ...
    if PPO_AVAILABLE:
        try:
            ppo_results = quick_train_ppo(episodes=episodes, max_steps_per_episode=max_steps_per_episode)
            if ppo_results:
    ...
    print("Results saved to results/quick_comparison_fixed/")


if __name__ == "__main__":
    import argparse
    
    parser = argparse.ArgumentParser(description='Compare RL algorithms for Spider Solitaire with fixed environments')
